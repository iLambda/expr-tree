...
module.exports = {
  ...
  compose: function(baseexpr, varname, expr) {
    // convert expr
    expr = _.isString(expr) ? expr : this.toRPN(expr)
    baseexpr = _.isString(baseexpr) ? baseexpr : this.toRPN(baseexpr)
    varname = _.isString(varname) ? varname : null

    return baseexpr.trim().replace(new RegExp(varname.trim(), 'g'), expr.trim())
  },
